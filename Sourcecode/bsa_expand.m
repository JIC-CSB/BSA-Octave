...
#############################################################################################################
# bsa_expand, wrapper function that keeps adding background functions to
# the specified limit, instead of stopping when modelratio > 1 
# INPUT(8):
...
#####################################################################################################
function [normp,omega,omega_max] = bsa_expand(data,name,start,stop,nsamples,nbackg,interval,resultsfile)
  fid = fopen(resultsfile,"a");
  c = ["name ","probability ","omega ","stdevO ","period ","stdevP "];
  fprintf(fid,"%s\n",c);
  fclose(fid);
  [normp,omega,omega_max] = bsa_modelratio_expand(data,name,start,stop,nsamples,nbackg,interval,resultsfile);
endfunction