...
#############################################################################################################
# bsa_modelratio_expand, part of model development, version that does not stop when ratio >1
####################################################################################################
function [normp,omega,omega_max] = bsa_modelratio_expand(data,name,start,stop,nsamples,nbackg,interval,fid)
  for (i=0:nbackg)
    if (i==nbackg)
...
#      if (ratio > 1) 
#	[normp,omega,omega_max] = runchannel (data,name,start,stop,nsamples,i,interval,fid);
#	break;
 #     else
	GL1_1 = GL1;
	GL2_1 = GL2;
